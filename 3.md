...
2.  **Полиморфизм JSON:** Класс `ClientRequest` использует аннотации Jackson для определения типа сообщения на основе поля `type`. . Это позволяет контроллеру принимать один базовый класс, а Spring сам создаст нужный наследник.
3.  **Виртуальные потоки:** В Java 21 они позволяют эффективно обрабатывать тысячи одновременных WebSocket соединений, так как блокировки ввода-вывода (отправка данных в сокет) становятся очень дешевыми.
