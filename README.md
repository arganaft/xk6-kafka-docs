...
```JavaScript
 const reader = new Reader({ brokers: ["localhost:9092"], topic: "my-topic", maxWait: "5s", // Максимальное время ожидания сообщений });
```
...
```JavaScript
 try { let messages = reader.consume({ limit: 10, }); } catch (error) { // Обработка ошибок таймаута или соединения console.error(error); }
```
