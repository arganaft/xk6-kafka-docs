...
## Опыт Pinterest: Масштабирование до 15,000 RPS
https://medium.com/pinterest-engineering/pinterests-analytics-as-a-platform-on-druid-part-1-of-3-9043776b7b76
Одним из наиболее показательных примеров является внедрение Druid в Pinterest для управления рекламной аналитикой. Изначально система на базе HBase не справлялась с ростом нагрузки и имела задержки обновления данных до 24 часов. Переход на Druid позволил компании достичь следующих показателей :Пиковая нагрузка: До оптимизации система обрабатывала пики до 15,000 запросов в секунду (RPS).Оптимизация QPS: За счет внедрения пакетной обработки сущностей (batching) в запросах (от 1000 до 2000 сущностей в одном фильтре IN) удалось снизить нагрузку на бэкенд в 10 раз, сохраняя при этом ту же логическую пропускную способность.Стабильность задержки: В кластере из более чем 2000 узлов 99% запросов выполняются менее чем за 250 мс.Для поддержания таких QPS Pinterest использует сложную трехуровневую систему исторических узлов: «горячий» уровень на дорогих вычислительных инстансах (AWS R-серия) обеспечивает хранение всех активных сегментов в оперативной памяти (через mmap и Page Cache), что исключает задержки на дисковый ввод-вывод. 1 
...
## Ресурсная эффективность: Пропускная способность на ядро RisingWave для сравнения потребления ресурсов:
| Запрос | Характер нагрузки | RisingWave (kr/s) | Apache Flink (kr/s) | Преимущество (RW / Flink) | Пропускная способность на ядро RisingWave (kr/s/ядро) | Пропускная способность на ядро Apache Flink (kr/s/ядро)* |
|--------|------------------|------------------|---------------------|---------------------------|------------------------------------------------------|--------------------------------------------------------|
| Q3     | Simple Join      | 338              | 140                 | 2.4x                      | 97.35                                                | 40.3                                                   |
| Q7     | Complex Join     | 313              | 88                  | 3.5x                      | 99.37                                                | 27.9                                                   |
| Q4     | Complex Join + Aggregation | 175      | 52                  | 3.3x                      | — (нет прямого аналога во 2й таблице)                | —                                                      |
| Q5     | Window Aggregation | 451            | 210                 | 2.1x                      | —                                                    | —                                                      |
| Q9     | Multi-way Join   | 285              | 90                  | 3.1x                      | —                                                    | —                                                      |
| Q17    | Complex Aggregation | 225            | 88                  | 2.5x                      | —                                                    | —                                                      |
| Q20    | Stateful Join    | 410              | 165                 | 2.4x                      | —                                                    | —                                                      |
